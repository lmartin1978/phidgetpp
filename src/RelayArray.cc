...
const std::vector<int> RelayArray::GetStates()
{
   std::vector<int> states;
   for(unsigned int i = 0; i < channels.size(); i++)
      states.push_back(channels[i]->GetState());
   return states;
}

PhidgetReturnCode RelayArray::SetStates(const std::vector<int> newStates)
{
   if(newStates.size() != channels.size())
      return EPHIDGET_INVALID;
   else {
      for(unsigned int i = 0; i < newStates.size(); i++){
         PhidgetReturnCode ret = channels[i]->SetState(newStates[i]);
         if(ret != EPHIDGET_OK)
            return ret;
      }
   }
